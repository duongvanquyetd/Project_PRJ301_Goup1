...
//lý do mà nó dư một khoảng trắng phía sau theo t nghĩ thì là do lúc tạ bảng mình tạo 255 ký tự nên nó dư ra nhiêu nên phải dùng trim vào xóa khoảng trắng
    ...
    public List<PersonDTA> getAllPerson() {
        try {
            Connection con = DBUtils.getConnection();
            ...
            ResultSet rs = stm.executeQuery();
            List<PersonDTA> list = new ArrayList();
            while (rs.next()) {
                list.add(new PersonDTA(rs.getString("PersonID").trim(), rs.getString("Name").trim(), rs.getString("Gender").trim(), rs.getString("Phone").trim(), rs.getString("RoleID").trim()));
            }
            return list;
        } catch (Exception e) {
        }
